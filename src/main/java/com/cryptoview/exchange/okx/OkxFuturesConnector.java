...
public class OkxFuturesConnector extends AbstractWebSocketConnector {
    ...
    protected void preloadVolumes(List<String> symbols) {
        log.info("[OKX:FUTURES] Pre-loading volumes for {} symbols...", symbols.size());
        int loaded = 0;
                ...
                try {
                    ...
                    try (Response response = httpClient.newCall(request).execute()) {
                        if (response.isSuccessful() && response.body() != null) {
                            ...
                            volumeTracker.seedVolume(symbol, Exchange.OKX, MarketType.FUTURES, totalVolume);
                            loaded++;
                        }
                    }
                } catch (Exception e) {
                    log.debug("[OKX:FUTURES] Failed to preload volume for {}: {}", instId, e.getMessage());
                }
            }
            try { Thread.sleep(500); } catch (InterruptedException e) { Thread.currentThread().interrupt(); break; }
        }
        log.info("[OKX:FUTURES] Pre-loaded volume for {}/{} symbols", loaded, symbols.size());
    }
