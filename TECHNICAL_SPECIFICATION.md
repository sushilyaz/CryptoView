...
| MEXC | ✅ | ❌ | USDT | Активна |
| Hyperliquid | ❌ | ✅ | USDC | Активна |
| Lighter | ✅ | ✅ | USDT | Требует проверки |
...
- **Rate limiter:** очередь сообщений (~20 msg/sec), retry с backoff при 429
...
- **Глубина:** 20 уровней (partial depth snapshot, без merge)
...
- **Orderbook channel:** `books5` (5 уровней, snapshot каждые 100ms)
- **Trade channel:** `trades`
- **Глубина:** 5 уровней (books5). `books50-l2-tbt` имеет лимит подписок и отклоняет при превышении

### 5.4 Bitget
- **Spot/Futures WebSocket:** `wss://ws.bitget.com/v2/ws/public` (v2 API)
