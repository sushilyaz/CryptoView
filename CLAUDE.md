...
- **SNAPSHOT_LIMIT:** Spot=5000 (вес 250, макс глубина), Futures=1000 (вес 20)
...
2. Запуск `fetchSnapshotsForAll()` в отдельном virtual thread — Spot: 1 req/2sec, Futures: 1 req/sec
...
3. Фоновый `refetchWorker` thread последовательно обрабатывает очередь (1 символ за раз, с паузой)
4. Это исключает каскадные REST-запросы при массовых gap (было причиной snapshot storm бага)
...
### Spot snapshot depth увеличена до 5000
**Было:** limit=1000 (вес 100). **Стало:** limit=5000 (вес 250, максимум Binance).
Скорость fetch: 1 req / 2 сек (безопасно для лимита 6000 weight/min). Полная инициализация ~15 мин для 440 символов.
