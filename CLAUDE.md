...
- **SNAPSHOT_LIMIT:** Spot=1000 (вес 100), Futures=1000 (вес 20)
...
2. Запуск `fetchSnapshotsForAll()` в отдельном virtual thread — ходит по 1 символу/сек
...
**КРИТИЧНО:** `handleDepthUpdate()` НЕ должен запускать `Thread.startVirtualThread(() -> fetchAndApplySnapshot())` для неинициализированных символов. Это было причиной бага — сотни одновременных REST-запросов → таймауты. Единственное место, где допустим параллельный запрос — gap re-initialization (разрыв последовательности), с задержкой 1 сек.
